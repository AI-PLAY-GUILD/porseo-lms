...
export async function POST(req: Request) {
    ...
    if (Number.isNaN(timestampMs) || Math.abs(Date.now() - timestampMs) > TIMESTAMP_TOLERANCE_MS) {
        console.error("Zoom webhook timestamp out of tolerance");
        return NextResponse.json({ error: "Request expired" }, { status: 401 });
    }

    const message = `v0:${timestamp}:${body}`;
    const hashForVerify = crypto.createHmac("sha256", ZOOM_WEBHOOK_SECRET).update(message).digest("hex");
    ...
    if (payload.event === "endpoint.url_validation") {
        ...
        const encryptedToken = crypto.createHmac("sha256", ZOOM_WEBHOOK_SECRET).update(plainToken).digest("hex");

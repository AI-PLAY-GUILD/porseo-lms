        ...
        <section id="team" className="py-16 md:py-24 bg-white relative overflow-hidden">
            <div className="container mx-auto px-4 relative z-10">
                <div className="text-center mb-20">
                    <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-6 text-black tracking-tighter" style={{ fontFamily: 'var(--font-jp)' }}>
                        コミュニティ運営チーム
                    </h2>
                    ...
                    {team.map((member, index) => (
                        ...
                        >
                            ...
                            <div className="bg-white border-4 border-black p-8 flex flex-col items-center text-center relative z-10 h-full rounded-lg">
                                {/* Avatar */}
                                <div className={`w-32 h-32 bg-gray-100 border-4 border-black flex items-center justify-center text-6xl mb-6 overflow-hidden rounded-2xl`}>
                                    {member.imageUrl ? (
                                        <Image
                                            src={member.imageUrl}
                                            alt={member.name}
                                ...
                                <h3 className="text-3xl font-black text-black mb-2 uppercase tracking-tight" style={{ fontFamily: 'var(--font-jp)' }}>
                                    {member.name}
                                </h3>
                                <div className="inline-block bg-black text-white px-3 py-1 font-bold text-sm mb-6 rounded-md">
                                    {member.role}
                                </div>
                                <p className="text-black font-bold leading-relaxed mb-8 flex-grow text-sm sm:text-base" style={{ fontFamily: 'var(--font-jp)' }}>
                                    {member.description}
                                </p>

                                {/* Social Links */}
                                <div className="flex gap-4 mt-auto">
                                    <Link href={member.twitterUrl} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-black text-white border-2 border-black flex items-center justify-center hover:bg-gray-800 transition-colors hover:translate-y-1 rounded-md">

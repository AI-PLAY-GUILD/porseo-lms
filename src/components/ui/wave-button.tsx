...
export function WaveButton({ href, text, hoverText, className, variant = "primary", onClick }: WaveButtonProps) {
    // Base styles
    const baseStyles =
        "overflow-hidden relative w-40 p-2 h-12 border-none rounded-md text-base font-bold cursor-pointer z-10 group shadow-md";

    // Variant styles
    const variantStyles = variant === "primary" ? "bg-black text-white" : "bg-white text-black border-2 border-black";

    const ButtonContent = () => (
        <button className={cn(baseStyles, variantStyles, className)} onClick={onClick} type="button">
            <span className="relative z-10 group-hover:opacity-0 transition-opacity duration-300 block">{text}</span>
